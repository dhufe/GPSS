...
def burst_pulse_train ( tpw, NPulses, f ):
    a = np.sin(np.pi*f*tpw)/(np.pi*f*tpw)
    b = np.sin(np.pi*f*NPulses*( 2*tpw ))/( np.sin( np.pi*f*( 2*tpw )))
    dBurst = np.abs(a*b)
    return dBurst/np.amax(dBurst)
...
#    GPSSPlot.PlottingSourceConfiguration(Xs, Ys, Zs)
    I0 = 1.0 * A_Burst[idx] * A_TA[idx] / Xs.size 
